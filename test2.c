...
bool verificando_ascoisa(int **front, int rainha, int col, int tam){
...
bool resolve_junin(int **front,int col,int tam){
    if(col >=tam)
    ...
    for(int i=0;i<tam;i++){
        if(verificando_ascoisa(front,i,col,tam)){
            front[i][col] = 1;  //colocando a rainha na posição (i,col)

            if(resolve_junin(front,col+1,tam)){
...
int main(){
    ...
    for(int i=0;i<tam;i++){
        free(front[i]);   //liberando a memoria do tabuleiro, não usei função pq é uma matriz simples
    }
    free(front);
